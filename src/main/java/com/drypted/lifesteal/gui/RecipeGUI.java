...
public class RecipeGUI {
    ...
    public static void openMainMenu(ServerPlayer player) {
        ...
        for (int i = 0; i < 27; i++) container.setItem(i, createNamed(Items.STAINED_GLASS_PANE.gray(), " "));

        container.setItem(HEART_BUTTON_SLOT, withName(ServerItemHelper.createHeart(), "§cConfigure Heart Recipe"));
        container.setItem(BEACON_BUTTON_SLOT, withName(ServerItemHelper.createReviveBeacon(), "§bConfigure Revive Beacon Recipe"));
    ...
    public static void openRecipeEditor(ServerPlayer player, String target) {
        ...
        for (int i = 0; i < CONTAINER_SIZE; i++) container.setItem(i, createNamed(Items.STAINED_GLASS_PANE.gray(), " "));
        ...
        container.setItem(4, createNamed(Items.BOOK, "§eClick an item in your inventory to select it, then click the grid to place it"));
        ...
        container.setItem(SAVE_SLOT, createNamed(Items.EMERALD_BLOCK, "§a§lSAVE RECIPE"));
        container.setItem(BACK_SLOT, createNamed(Items.ARROW, "§e§lGO BACK"));
        container.setItem(DISCARD_SLOT, createNamed(Items.REDSTONE_BLOCK, "§c§lDISCARD CHANGES"));
    ...
    public static void updateBrush(EditorContainer container) {
        ...
        if (selected.isEmpty()) {
            container.setItem(BRUSH_SLOT, createNamed(Items.STAINED_GLASS_PANE.lightGray(),
                    "§7Selected: none §8(click an item in your inventory)"));
        } else {
            container.setItem(BRUSH_SLOT, withName(selected.copyWithCount(1), "§aSelected ingredient"));
        }
    }

    private static ItemStack createNamed(Item item, String name) {
